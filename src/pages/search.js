        ...
        load(`launch?limit=300&ordering=-net&search=${term}${($query.type == "failures") ? "&status=4" : ""}`, function (data) {
            ...
            if (data.results.length) {
                ...
                if (data.results.length == 1) {
                    ...
                    document.getElementById("next").innerHTML = '<div class="col s12"><div class="card"><div class="card-stacked"><div class="card-content"><h3 class="header black-text"><a class="tooltipped" data-tooltip="More info" href="/#rocket=' +launch.rocket.configuration.id + '">' + launch.name.replace(" | ", "</a></h3><h2>") + '</h2><a class="chip tooltipped" data-tooltip="' + launch.net + '"><i class="far fa-clock"></i>' + ReadableDateString(launch.net) + '</a><h4 id="countdown-' + launch.id + '">' + getLongStatusName(launch.status.id).name + '</h4></div><div class="card-action"><a class="waves-effect waves-light btn hoverable" href="/#launch=' + (launch.launch_library_id || launch.slug) + '">Details</a></div></div></div>';
                } else {
                    ...
                    for (let launch of data.results) {
                        if (Date.parse(launch.net) < $today) {
                            $past += '<div class="col s12 m6"><div class="card"><div class="card-content"><h5 class="header truncate"><a class="tooltipped" data-tooltip="More info" href="/#rocket=' + launch.rocket.configuration.id + '">' + launch.name.replace(" | ", "</a></h5><h4 class='header truncate'>") + '</h4><a class="chip tooltipped" data-tooltip="' + launch.net + '"><i class="far fa-clock"></i>' + ReadableDateString(launch.net) + "</a><h5>" + getLongStatusName(launch.status.id) + '</h5></div><div class="card-action"><a class="waves-effect waves-light btn hoverable" href="/#launch=' + (launch.launch_library_id || launch.slug) + '">Watch</a></div></div></div>'
                        } else {
                            if (Date.parse(launch.net) > $today) {
                                $future = '<div class="col s12 m6"><div class="card"><div class="card-content"><h5 class="header truncate"><a class="tooltipped" data-tooltip="More info" href="/#rocket=' + launch.rocket.configuration.id + '">' + launch.name.replace(" | ", "</a></h5><h4 class='header truncate'>") + '</h4><a class="chip tooltipped" data-tooltip="' + launch.net + '"><i class="far fa-clock"></i>' + ReadableDateString(launch.net) + "</a><h5>" + getLongStatusName(launch.status.id) + '</h5></div><div class="card-action"><a class="waves-effect waves-light btn hoverable" href="/#launch=' + (launch.launch_library_id || launch.slug) + '">Details</a></div></div></div>' + $future;
                            }
                            if (launch.status.id == 1 || launch.status.id == 5 || launch.status.id == 6) {
                                document.getElementById("next").innerHTML = '<div class="col s12"><div class="card"><div class="card-stacked"><div class="card-content"><h3 class="header black-text"><a class="tooltipped" data-tooltip="More info" href="/#rocket=' + launch.rocket.configuration.id + '">' + launch.name.replace(" | ", "</a></h3><h2>") + '</h2><a class="chip tooltipped" data-tooltip="' + launch.net + '"><i class="far fa-clock"></i>' + ReadableDateString(launch.net) + '</a><h4 id="countdown-' + launch.id + '">' + getLongStatusName(launch.status.id) + '</h4></div><div class="card-action"><a class="waves-effect waves-light btn hoverable" href="/#launch=' + (launch.launch_library_id || launch.slug) + '">Details</a></div></div></div>';
